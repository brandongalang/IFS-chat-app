...
- Client-safe (browser):
  - import from `@/lib/data/parts-client`
...
- Under the hood:
  - `@/lib/data/parts.ts` is marked server-only, exposes richer logic (logging, snapshots), and should only be imported by the entrypoints below.
  - Snapshot logic lazily imports storage adapters on the server and throws if called on the client.
- Naming conventions:
  - Client-facing entrypoints use the `-client.ts` suffix (e.g., `parts-client.ts`).
  - Server entrypoints use the `.server.ts` suffix (e.g., `parts.server.ts`) so Next.js can enforce server-only execution.
  - Keep shared implementations like `parts.ts` internal—import the appropriate client or server wrapper instead.
...
Migration tips:
- If a client file previously imported from `@/lib/data/parts`, switch to `@/lib/data/parts-client`.
- If a server file needs snapshots or writes/action logging, import from `@/lib/data/parts.server`.
- When in doubt: client → `parts-client`; server → `parts.server`.
