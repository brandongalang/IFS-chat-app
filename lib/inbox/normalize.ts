...
function coerceNotificationPayload(candidate: unknown): NotificationMessage | null {
  ...
  const link = isRecord(candidate.link)
    ? {
        ...
        target: normalizeTarget(safeString(candidate.link.target)),
...
function coerceInboxCta(candidate: unknown): InboxCTA | null {
  ...
  const target = normalizeTarget(safeString(candidate.target))
  const analyticsTag = safeString(candidate.analyticsTag) ?? undefined
  return {
    label,
    href,
    actionId,
...
function normalizeTarget(candidate: string | null): '_self' | '_blank' | undefined {
  if (candidate === '_blank') return '_blank'
  if (candidate === '_self') return '_self'
  return undefined
}
