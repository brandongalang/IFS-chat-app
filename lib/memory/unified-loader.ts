 ...
 */
export async function loadUnifiedUserContext(
  ...
  try {
    ...
    // Load both in parallel for efficiency - UserMemory reconstruction is currently disabled
    const [contextCache, recentTimeline] = await Promise.all([
      loadUserContextCache(supabase, normalizedUserId),
      loadRecentTimelineEvents(supabase, normalizedUserId),
    ]);

    const userMemory: UserMemory = INITIAL_USER_MEMORY;
 ...
 */
async function loadUserContextCache(
  ...
  if (error) {
    console.warn('[UnifiedLoader] Failed to load user_context_cache', {
      userId,
      error: error.message,
    });
    return null;
...
async function loadRecentTimelineEvents(
  ...
  if (error) {
    console.warn('[UnifiedLoader] Failed to load timeline_display', {
      userId,
      error: error.message,
    });
    return [];
