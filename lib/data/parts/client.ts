// Client-safe parts functions (browser Supabase client)
export {};
...
const fromV2RelationshipType = (type: string) => {
  if (type === 'polarized') return 'conflicted';
  if (type === 'allied') return 'active';
  return type;
};
...
export type PartsLiteDependencies = {
...
export async function searchParts(
  ...
  return runWithClient(searchPartsSchema, input, deps, async (validated, supabase) => {
    ...
    return rows.map((row) => mapPartsDisplayRowToV2(row));
  });
}
...
export async function getPartById(
  ...
  return runWithClient(getPartByIdSchema, input, deps, async (validated, supabase) => {
    ...
    return mapPartsDisplayRowToV2(data as PartsDisplayRow);
  });
}
...
export async function getPartRelationships(
  ...
  return runWithClient(getPartRelationshipsSchema, input, deps, async (validated, supabase) => {
    ...
    if (validated.relationshipType) {
      ...
      query = query.eq('event_subtype', validated.relationshipType);
    }
    ...
    if (validated.status) {
      filtered = filtered.filter(
        ({ context }) => (context.status ?? DEFAULT_RELATIONSHIP_STATUS) === validated.status
      );
    }
    ...
    if (validated.includePartDetails) {
      ...
      if (partIds.size > 0) {
        ...
        partsDetails = (parts ?? []).reduce<Record<string, { name: string; status: string }>>(
          (acc, part: any) => {
    ...
    return limited.map(({ rel, context, dynamics }) => {
      ...
      return {
        ...
        dynamics: dynamics.map((d) => ({ observation: d, context: '' })), // Map string observations to dynamics
