...
useEffect(() => {
  ...
  if ((messages?.length ?? 0) === 0) {
    ...
    if (inboxContext) {
      ...
      generateAssistantMessage({
        systemContext: `User is starting a new chat session. 
          Current time: ${new Date().toLocaleString()}
          Recent activity: Check user's recent patterns and check-ins.
          Generate a warm, contextual greeting that invites them to share what's on their mind.`,
      });
    }
  }
...
Be curious about the deeper dynamics at play. Reference specific details from the observation.`;
  } else {
    return `The user just reviewed and DISAGREED with the following observation from their inbox:
