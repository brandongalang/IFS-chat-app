...
import { z } from 'zod';
import { r as resolveUserId, a as dev } from '../dev.mjs';
...
async function getRecentSessions(input) {
  try {
    ...
    const { data, error } = await supabase.from("sessions").select("*").eq("user_id", userId).gte("start_time", lookbackDate.toISOString()).order("start_time", { ascending: false }).limit(validated.limit);
    if (error) return { success: false, error: `Database error: ${error.message}` };
    return { success: true, data: data || [], confidence: 1 };
  } catch (error) {
...
async function getRecentInsights(input) {
  try {
    ...
    const lookbackDate = /* @__PURE__ */ new Date();
    lookbackDate.setDate(lookbackDate.getDate() - validated.lookbackDays);
    const { data, error } = await supabase.from("insights").select("*").eq("user_id", userId).gte("created_at", lookbackDate.toISOString()).order("created_at", { ascending: false }).limit(validated.limit);
    if (error) return { success: false, error: `Database error: ${error.message}` };
    return { success: true, data: data || [], confidence: 1 };
  } catch (error) {
    const errMsg = error instanceof Error ? dev.verbose ? error.stack || error.message : error.message : "Unknown error occurred";
...
export { getActiveParts, getActivePartsTool, getPolarizedRelationships, getPolarizedRelationshipsTool, getRecentInsights, getRecentInsightsTool, getRecentSessions, getRecentSessionsTool, insightResearchTools };
