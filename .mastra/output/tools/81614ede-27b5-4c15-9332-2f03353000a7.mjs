...
async function proposePartSplit(input) {
  const supabase = getSupabaseClient();
  const { error, data } = await supabase.from("part_change_proposals").insert({
  ...
  if (error) {
    const isIdem = error?.message?.includes("uq_part_change_proposals_idem");
    if (!isIdem) return { success: false, error: `Failed to propose split: ${error.message}` };
  }
  return { success: true, data };
}
async function proposePartMerge(input) {
  const supabase = getSupabaseClient();
  const { error, data } = await supabase.from("part_change_proposals").insert({
    user_id: input.userId,
    type: "merge",
...
async function approveProposal(input) {
  const supabase = getSupabaseClient();
  const { data: proposal, error } = await supabase.from("part_change_proposals").select("*").eq("id", input.proposalId).eq("user_id", input.userId).single();
  if (error || !proposal) {
  ...
  const { error: updErr, data: updated } = await supabase.from("part_change_proposals").update({ status: "approved", approved_at: (/* @__PURE__ */ new Date()).toISOString(), approved_by: input.approvedBy }).eq("id", input.proposalId).select("*").single();
...
async function rejectProposal(input) {
  ...
  const { error: updErr, data: updated } = await supabase.from("part_change_proposals").update({ status: "rejected", approved_at: (/* @__PURE__ */ new Date()).toISOString(), approved_by: input.approvedBy }).eq("id", input.proposalId).select("*").single();
  if (updErr) return { success: false, error: `Failed to reject: ${updErr.message}` };
  return { success: true, data: updated };
}
async function executeSplit(userId, proposalId) {
  const supabase = getSupabaseClient();
  const { data: proposal } = await supabase.from("part_change_proposals").select("*").eq("id", proposalId).eq("user_id", userId).single();
  if (!proposal || proposal.type !== "split") throw new Error("Invalid split proposal");
  const payload = proposal.payload;
  const { data: parent, error: parentErr } = await supabase.from("parts").select("*").eq("id", payload.parentPartId).eq("user_id", userId).single();
  if (parentErr || !parent) throw new Error("Parent part not found");
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const inserts = payload.children.map((c) => ({
    user_id: userId,
    name: c.name,
    status: "emerging",
    ...
    visualization: parent.visualization || { emoji: "\u{1F917}", color: "#6B7280", energyLevel: 0.5 },
  ...
  const { data: children, error: childErr } = await supabase.from("parts").insert(inserts).select("*");
  if (childErr) throw childErr;
  const relationships = parent.relationships || {};
  const lineage = { ...relationships.lineage || {} };
  lineage["superseded_by"] = [...lineage["superseded_by"] || [], ...children.map((c) => c.id)];
  const updatedParent = await actionLogger.loggedUpdate(
    "parts",
    parent.id,
  ...
  const { error: execErr } = await supabase.from("part_change_proposals").update({ status: "executed", executed_at: now, executed_by: "agent" }).eq("id", proposalId);
  if (execErr) throw execErr;
  return { parent: updatedParent, children };
}
async function executeMerge(userId, proposalId) {
  const supabase = getSupabaseClient();
  const { data: proposal } = await supabase.from("part_change_proposals").select("*").eq("id", proposalId).eq("user_id", userId).single();
  if (!proposal || proposal.type !== "merge") throw new Error("Invalid merge proposal");
  const payload = proposal.payload;
  const { data: parts, error: partsErr } = await supabase.from("parts").select("*").eq("user_id", userId).in("id", payload.partIds);
  if (partsErr || !parts || parts.length < 2) throw new Error("Parts not found for merge");
  const canonical = parts[0];
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const updatedCanonical = await actionLogger.loggedUpdate(
    "parts",
    canonical.id,
...
const executeSplitTool = createTool({
  ...
  execute: async ({ context }) => {
    const { userId, proposalId } = context;
