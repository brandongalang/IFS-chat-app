...
import { searchParts as searchPartsData } from '@/lib/data/parts/agent';
...
export function createEvidenceTools(userId?: string) {
  const logEvidence = createTool({
    ...
    execute: async ({
      ...
      context: LogEvidenceInput;
      runtime?: { userId?: string };
    }): Promise<ToolResult> => {
      try {
        // const input = logEvidenceSchema.parse(context)
        const resolvedUserId = ensureUserId(userId, runtime);
        // TODO: Log evidence (memory v2 snapshots removed)
        return { success: true, data: { evidenceAdded: 0, userId: resolvedUserId } };
      } catch (error) {
        return {
          success: false,
  ...
  const findPatterns = createTool({
    ...
    execute: async ({
      ...
      try {
        ...
        // TODO: Fetch sessions from DB instead of storage (memory v2 snapshots removed)
